    ...
    foreach($table->childNodes as $subjTr){
      if($subjTrI > 0){
        ...
        if(!$isNewSubject && $needsGrades && !empty(trim($subjTr->nodeValue))){
          ...
          $gradeTrI = 0;
          foreach($gradeTable->childNodes as $gradeTr){
            if($gradeTrI > 0 && $gradeTrI != sizeof($gradeTable->childNodes) - 1){
              $grade = (object)[];

              $nameStr = trim($gradeTr->childNodes[2]->nodeValue);
              if(!empty($nameStr)){
                $grade->name = $nameStr;
              }

              $valueElement = $gradeTr->childNodes[4];
              $valueElI = 0;
              ...
              $pointsStr = trim(explode(":", $valueElement->textContent)[1]);
